...
#     )
    ...
    chain_json = (
        ChatPromptTemplate.from_template(
            ...
            '''
You are an AI assisstant thet splits long text into two parts. You have received a long text in markdown. Now you are going to split it into two parts and turn it into a json. Please follow these guidelines:
- The json should be in strict format like {"part1": "...", "part2": "..."}
- Choose the split location carefully. Only split between paragraphs.
...
Firstly, tell me where you are planning to make the split. Then, answer with the strict json format.
'''.strip(),
            template_format='jinja2'
        )
        | ChatOpenAI(temperature=0).configurable_fields(
    ...
    cache_id = str(chain_json) + str(chain_markdown) + page_text
    cached_result = simple_caching.get_text_cache(cache_dir, identifier=cache_id)
    if (cached_result is not None):
        return json.loads(cached_result)
...
'''
        ...
        logger.info(f'llm markdown {page_markdown}')
        ...
        logger.info(f'The original page:')
...
async def iterate_line_type_llm(
        context_iter: Generator[Tuple[str, List[str]], Any, None]) -> Generator[Tuple[str, int], Any, None]:
    ''' yields (line, category_index) '''
    

    for line, context in context_iter:

        # skip empty lines
        if (line.strip() == ''):
            yield (line, -1) # -1 for empty line
            continue

        cat = await llm_judge_line_removal(line, context)
...
async def atest():
    ...
    pages = list(pdf_text.extract_text_from_pdf('test.pdf', range(5)))
    import os
